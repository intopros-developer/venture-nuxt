    ...
    <div>
        <div>
            <section id="ourCulture"
                class="relative bg-cover bg-center bg-no-repeat pt-[50px] pb-32 lg:pt-[280px] lg:pb-[175px]"
                :style="{ backgroundImage: `url(${image})` }">
                <div class="absolute top-0 left-0 -z-0 h-full w-full"
                    style="background-image: linear-gradient(270deg, rgba(0, 0, 0, 0) 0%, #2b3746 100%)"></div>
                <div class="container relative z-10 px-10 lg:px-[97px] xl:px-4">
                    <h3 class="max-w-[800px] text-[28px] font-bold uppercase leading-[42px] text-white lg:text-[41px] lg:leading-[54px]"
                        v-html="title"></h3>
                </div>
            </section>
            <section class="relative -mt-24 pb-10 lg:-mt-[128px] lg:pb-16">
                <div class="container px-10 lg:px-[97px] xl:px-4">
                    <div class="mb-[33px] grid grid-cols-1 gap-4 sm:grid-cols-2 lg:mb-[85px] lg:grid-cols-4 lg:gap-5">
                        <div v-for="(value, i) in values" :key="i">
                            <div
                                class="flex h-auto items-center justify-center bg-primary/80 px-4 py-9 lg:min-h-[120px] lg:py-[30px] lg:px-4 xl:h-auto xl:px-[26px]">
                                <p class="text-sm font-bold capitalize leading-4 text-white lg:text-lg lg:leading-[30px]">{{
                                    value.attributes.title }}</p>
                            </div>
                            <div
                                class="bg-white px-6 py-5 shadow-[0px_3px_6px_rgba(0,0,0,0.16)] md:min-h-[178px] lg:px-6 lg:py-5">
                                <p class="text-sm font-medium text-black xl:leading-[22px]">{{ value.attributes.description
                                }}</p>
                            </div>
                        </div>
                    </div>
