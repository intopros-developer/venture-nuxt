<template>
    <div
        class="grid flex-none grid-cols-2 gap-3 sm:gap-[22px] md:w-full md:max-w-sm lg:max-w-[422px] xl:max-w-[660px] xl:gap-[34px]">
        <div class="rounded-xl bg-white shadow-[0px_1px_8px_rgba(0,161,241,0.12)]">
            <div class="flex items-center gap-2 border-b border-[#707070]/20 py-3 px-7">
                <p class="text-sm font-semibold leading-8 text-[#5CAFCE] sm:text-[28px] xl:text-[46px] xl:leading-[56px]">{{
                    chart1.avgCount }}</p>
                <p
                    class="text-left text-[10px] font-extrabold leading-[14px] -tracking-[0.03px] text-[#707070] sm:text-sm xl:text-[15px] xl:leading-[22px]">
                    {{ $t('industry_consultants') }}</p>
            </div>
...
export default {
    ...
    methods: {
        updateChart(service) {
            let chart1Md = parseInt(service.attributes.industryConsultant)
            let chart1p1 = parseInt(service.attributes.industryConsultant) / 2
            let chart1p2 = (chart1Md * 2) - chart1p1
             ...
             */
            let chart2Md = parseInt(service.attributes.businessPlansWritten)
            let chart2p1 = parseInt(service.attributes.businessPlansWritten) / 2
            let chart2p2 = (chart2Md * 2) - chart2p1
